...
export function meta({ params }: { params: { workId: string } }) {
  return [
    { title: `Work Detail - ${params.workId}` },
    {
      name: "description",
      content:
        "Get in touch with Charles Mbachu, a specialized Full Stack Web Developer and Creative Technologist.",
    },
  ];
}
...
export default function WorkDetail() {
  ...
  const selectedWork = WORKS.find(
    (work) =>
      work.client.split(" ").join("-").toLowerCase() === workId?.toLowerCase(),
  );
  ...
  const videoScale = useTransform(scrollYProgress, [0.1, 0.4, 0.7], [0.9, 1, 0.95]);
  ...
  return (
    <div
      ref={containerRef}
      className="bg-[#0a0a0a] text-white selection:bg-SoftApricot selection:text-black min-h-screen relative overflow-hidden"
    >
      ...
      >
        <h2 className="text-[30vw] font-black uppercase text-white leading-none whitespace-nowrap">
          {selectedWork?.client}
        </h2>
      </motion.div>
        ...
        <PageWrapper>
          <PageSection index={0}>
            ...
            >
              ...
              <h1 className="text-6xl md:text-8xl lg:text-[10rem] font-black uppercase tracking-tighter leading-[0.8] mb-12">
                {selectedWork?.client.split(" ").map((word, i) => (
                  <span key={i} className="block last:text-white/20">
                    {word}
                  </span>
                ))}
              </h1>
            ...
            <div className="max-w-[1350px] mx-auto px-6 md:px-12 pb-60 relative z-10">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
                ...
                <div className="lg:col-span-8">
                  ...
                  >
                    <div>
                      <span className="text-SoftApricot font-mono text-[10px] mb-8 block opacity-40 uppercase tracking-[0.4em]">
                        01 / OVERVIEW
                      </span>
                      <p className="text-2xl md:text-5xl text-white/90 leading-[1.2] font-light tracking-tight">
                      ...
                      <div className="flex flex-wrap gap-4">
                        {selectedWork?.stack?.map((item, i) => (
                          <motion.span
                            ...
                            className="px-6 py-3 rounded-full bg-white/5 border border-white/5 text-[10px] font-bold tracking-[0.2em] uppercase text-white/40 hover:border-SoftApricot/30 hover:text-white hover:bg-white/10 transition-all cursor-default"
                          >
                            {item}
                          </motion.span>
                        ))}
